...
const specShowcase = document.querySelector("#spec-showcase");
const specTriggers = Array.from(document.querySelectorAll("[data-spec-trigger]"));
const specPanels = Array.from(document.querySelectorAll("[data-spec-panel]"));

function showSpecPanel(specKey) {
  if (!specShowcase || !specPanels.length) {
    return;
  }

  specShowcase.hidden = false;

  specPanels.forEach((panel) => {
    const isActive = panel.dataset.specPanel === specKey;
    panel.hidden = !isActive;
    panel.classList.toggle("is-active", isActive);
  });

  specTriggers.forEach((trigger) => {
    const isActive = trigger.dataset.specTrigger === specKey;
    trigger.classList.toggle("is-selected", isActive);
    trigger.setAttribute("aria-pressed", String(isActive));
  });

  window.setTimeout(() => {
    specShowcase.scrollIntoView({ behavior: "smooth", block: "start" });
  }, 40);
}
...
specTriggers.forEach((trigger) => {
  ...
  trigger.addEventListener("click", () => {
    showSpecPanel(trigger.dataset.specTrigger);
  });
...
if (window.location.hash === "#spec-dath") {
  showSpecPanel("dath");
}

if (window.location.hash === "#spec-dlct") {
  showSpecPanel("dlct");
}
